 ...
 *
 */
@Component(value = "animatedGifWriter")
public class AnimatedGIFWriter implements ICreateAnimatedGif {

	private static Logger Logger = LoggerFactory.getLogger(AnimatedGIFWriter.class);
	// Fields
	private int codeLen;
	...
	/**
	 * A child is made up of a parent(or prefix) code plus a suffix color and
	 * siblings are strings with a common parent(or prefix) and different suffix
	 * colors
	 */
	...
	/**
	 * This is intended to be called after writing all the frames if we write an
	 * animated GIF frame by frame.
	 * 
	...
	/**
	 * This is intended to be called first when writing an animated GIF frame by
	 * frame.
	 * 
	 * @param os                  OutputStream for the animated GIF
	 * @param logicalScreenWidth  width of the logical screen. If it is less than or
	 *                            equal zero, it will be determined from the first
	 *                            frame
	...
	/**
	 * Java port of C Implementation of Wu's Color Quantizer (v. 2) (see Graphics
	 * Gems vol. II, pp. 126-133) Author: Xiaolin Wu Dept. of Computer Science Univ.
	 * of Western Ontario London, Ontario N6A 5B7 wu@csd.uwo.ca
	 * 
	 * Algorithm: Greedy orthogonal bipartition of RGB space for variance
	 * minimization aided by inclusion-exclusion tricks. For speed no nearest
	 * neighbor search is done. Slightly better performance can be expected by more
	 * sophisticated but more expensive versions.
	 * 
	 * The author thanks Tom Lane at Tom_Lane@G.GP.CS.CMU.EDU for much of additional
	 * documentation and a cure to a previous bug.
	 * 
	...
	private static class WuQuant {
		...
		/*
		 * At conclusion of the histogram step, we can interpret wt[r][g][b] = sum over
		 * voxel of P(c) mr[r][g][b] = sum over voxel of r*P(c) , similarly for mg, mb
		 * m2[r][g][b] = sum over voxel of c^2*P(c) Actually each of these should be
		 * divided by 'size' to give the usual interpretation of P() as ranging from 0
		 * to 1, but we needn't do that here.
		 */

		/*
		 * We now convert histogram into moments so that we can rapidly calculate the
		 * sums of the above quantities over any desired box.
		 */
		...
		/*
		 * The next two routines allow a slightly more efficient calculation of Vol()
		 * for a proposed subbox of a given box. The sum of Top() and Bottom() is the
		 * Vol() of a subbox split in the given direction and with the specified new
		 * upper bound.
		 */
		...
		/*
		 * We want to minimize the sum of the variances of two subboxes. The sum(c^2)
		 * terms can be ignored since their sum over both subboxes is the same (the sum
		 * for the whole box) no matter where we split. The remaining terms have a minus
		 * sign in the variance formula, so we drop the minus sign and MAXIMIZE the sum
		 * of the two terms.
		 */
	...
	private static class InverseColorMap {
		...
		public int getNearestColorIndex(int red, int green, int blue) {
			return invColorMap[(((red >> bitsDiscarded) << (bitsReserved << 1)))
					| ((green >> bitsDiscarded) << bitsReserved) | (blue >> bitsDiscarded)] & 0xff;
		}
		...
		public void createInverseMap(int no_of_colors, int[] colorPalette) {
			...
			for (int i = 0; i < no_of_colors; i++) {
				...
				/**
				 * We start from the origin (0,0,0) of the quantized colors, calculate the
				 * distance between the cell center of the quantized colors and the current
				 * color map entry as follows: (rcenter * x + x/2) - red, where rcenter is the
				 * center of the Quantized red color map entry which is 0 since we start from 0.
				 */
