...
public class RequestCacheManager {
	 ...
	 * Merge similar request only when cache is enable
	 * 
	 * @param similarRequests
	 * @param collectRequest
	 */
	private void mergeRequest(Set<Request> similarRequests, CollectRequest collectRequest) {
		if (similarRequests != null && !similarRequests.isEmpty()
				&& (collectRequest.getKeywordAnyList() == null || collectRequest.getKeywordAnyList().isEmpty()) && collectRequest.isCached()) {
	...
	private Set<Request> exactRequests(Set<Request> request, CollectRequest collectRequest) {
		...
		// if one keywords, no user => request 1 keyword + 1 users => reuse 1 keywords
...
//				collectRequest.getKeywordAnyList(), collectRequest.getBannedWords(), collectRequest.getUserList());
		Set<Request> mergedRequest = collectService.requestConstainsCriteriasWithPriority(
				collectRequest.getKeywordList(), collectRequest.getKeywordAnyList());
		Set<Request> machingRequests = mergeCriteria(mergedRequest);
